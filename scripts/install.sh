#!/usr/bin/env sh
# Instala AI Usage en macOS o Linux con el último release de GitHub.
# Uso:
#   curl -fsSL https://raw.githubusercontent.com/RagnArUCi/ai-usage-bar/main/scripts/install.sh | sh
set -eu

REPO="RagnArUCi/ai-usage-bar"
...
case "$OS" in
  Darwin)
    ...
    trap 'rm -rf "$tmp"' EXIT
    log "Descargando $url"
    curl -fL "$url" -o "$tmp/ai-usage.dmg"

    mnt="$(hdiutil attach "$tmp/ai-usage.dmg" -nobrowse -readonly | tail -n1 | awk '{print $NF}')"
