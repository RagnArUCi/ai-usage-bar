...
/**
 * Marca de la app: tres barras de medidor de alturas distintas. Neutral a
 * propósito — el sunburst es de Claude y aquí conviven varios proveedores.
 */
function drawBars(rgba, width, height, opts) {
  const { cx, cy, size, color } = opts;
  const [cr, cg, cb] = color;
  const heights = [0.5, 1.0, 0.72];
  const barW = size * 0.2;
  const gap = size * 0.14;
  ...
  const bars = heights.map((h, i) => ({
    left: x0 + i * (barW + gap),
    right: x0 + i * (barW + gap) + barW,
    top: bottom - size * h,
    bottom,
  }));

  const inside = (px, py, b) => {
    // Rectángulo con los extremos redondeados.
    if (px < b.left || px > b.right) return false;
    if (py < b.top || py > b.bottom) return false;
    const midTop = b.top + radius;
    const midBottom = b.bottom - radius;
    if (py >= midTop && py <= midBottom) return true;
    const cyy = py < midTop ? midTop : midBottom;
    const cxx = (b.left + b.right) / 2;
    return Math.hypot(px - cxx, py - cyy) <= radius;
  };
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let sy = 0; sy < SS; sy++) {
        for (let sx = 0; sx < SS; sx++) {
          const px = x + (sx + 0.5) / SS;
          const py = y + (sy + 0.5) / SS;
          if (bars.some((b) => inside(px, py, b))) hit++;
        }
      }
      if (!hit) continue;
      const cov = hit / (SS * SS);
...
module.exports = { drawSunburst, drawBars, fillRoundedRect };
